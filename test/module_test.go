...
func TestTerraformModule(t *testing.T) {
	...
	dataStreamMap := terraform.OutputMap(t, terraformOptions, "data_stream")
    dataStreamName, exists := dataStreamMap["name"]
    assert.True(t, exists, "'name' key should exist in data_stream output")
    assert.Contains(t, dataStreamName, "cloudwatch-to-s3", "Data stream name should contain 'cloudwatch-to-s3'")
